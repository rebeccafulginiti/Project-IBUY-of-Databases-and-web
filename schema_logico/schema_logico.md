...
* Products(__product_id__, timestamp_put_on_sale, name, --quantity--, price, description, type, *username_vendor*, --byte--, --weight_in_kg--)
  » * foreign key (username_vendor) references utente (username)
* Purchases(__*username_vendor*__, __*username_buyer*__, __*product_id*__, __timestamp_transaction__)
  » * foreign key (username_vendor) references utente (username)
  » * foreign key (username_buyer) references utente (username)
  » * foreign key (product_id) references prodotto (product_id)
* Follows(__*segue*__, __*seguito*__)
  » * foreign key (follower) references utente (username)
  » * foreign key (following) references utente (username)